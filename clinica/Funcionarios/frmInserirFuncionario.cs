...
namespace clinica.Funcionarios
{
    public partial class frmInserirFuncionario : Form
    {
        ...
        public frmInserirFuncionario(frmPrincipal frm)
        {
            InitializeComponent();
            frm1 = frm;
        }
        ...
        private void inserirFuncionario()
        {
            try
            {
                ...
                string query = "INSERT INTO funcionarios(nome, cartaoCidadao, dataNasc, morada, localidade, telefone, email, vencimentoBase, tipoContrato, limiteContrato) " +
                    "VALUES(@nome, @cc, @dataNasc, @morada, @localidade, @telefone, @email, @vencimento, @tipoContrato, @limiteContrato)";
                ...
                using (SqlCommand comm = new SqlCommand(query, baseDados.conn))
                {
                        ...
                        maskedTxtNumTelefone.Text != "" && cbTipoContrato.Text != "")
                    {
                        if (validarNome() == true && validarCC() == true && validarNumTelefone() == true && validarEmail() == true && validarMorada() == true && validarLocalidade() == true && validarVencimento() == true)
                        {
                            if (verificarFuncionario(maskedTxtNumCC.Text) == true)
                            {
                                ...
                                frm1.numFuncionarios();
                                MessageBox.Show($"O funcionário {txtNome.Text} foi inserido com sucesso!", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                limparCampos();
                            }
                            else
                            {
                    ...
                    {
                        MessageBox.Show("Não é possível introduzir um funcionário com campos incompletos!", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void limparCampos()
        {
            ...
            txtEmail.Text = "";
            maskedTxtVencimento.Text = "";
            cbTipoContrato.Text = "";
            dateTimePickerDuracaoContrato.Value = DateTime.Now;
        }
        ...
        private void btnCancelar_Click(object sender, EventArgs e)
        {
            ...
            msg = MessageBox.Show("Desejas cancelar a inserção de um novo funcionário?", "Confirmar", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
